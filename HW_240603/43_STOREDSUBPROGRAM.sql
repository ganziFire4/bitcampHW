...
CREATE OR REPLACE PROCEDURE P_MAJOR_AVG_RES IS
    CURSOR MAJ IS
        SELECT ST.MAJOR
            , ROUND(AVG(SC.RESULT), 2) AS NEW_RES
            FROM STUDENT ST
            JOIN SCORE SC
              ON ST.SNO = SC.SNO
            GROUP BY ST.MAJOR;
BEGIN
    FOR TEMP IN MAJ LOOP
...
CREATE OR REPLACE FUNCTION F_GET_VACATION_CNT(
 ...
 IS 
    STUDY_YEAR NUMBER;
 BEGIN 
    STUDY_YEAR := TRUNC(MONTHS_BETWEEN(SYSDATE, HIREDATE) / 60);
    RETURN STUDY_YEAR;
END;
/
...
SELECT PNO
    , PNAME
    , HIREDATE
    , F_GET_VACATION_CNT(HIREDATE)AS STUDY
    FROM PROFESSOR;
...
CREATE OR REPLACE TRIGGER TR_BORROW
BEFORE
INSERT ON TR_BORROW
